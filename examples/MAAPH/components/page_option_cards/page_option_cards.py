...
class ExampleOptionCards(SiPage):
    def __init__(self, *args, **kwargs):
        ...
        # 在创建header_button后，连接点击信号
        header_button.clicked.connect(self.toggle_body_visibility)
    ...
    def toggle_body_visibility(self):
        body_container = self.option_card_plane_beginning.body()
        new_visible = not body_container.isVisible()

        if new_visible:
            # 恢复之前保存的原始高度，如果没有保存则使用 sizeHint 提供的推荐高度
            original_height = getattr(body_container, '_original_height', body_container.sizeHint().height())
            body_container.setFixedHeight(original_height)
            body_container.setVisible(True)
        else:
            # 第一次隐藏时保存原始高度
            if not hasattr(body_container, '_original_height'):
                body_container._original_height = body_container.height()
            body_container.setFixedHeight(0)
